    ...
    <div class="max-w-full mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Reportes - Rentabilidad por Vendedor</h1>
                    <p class="text-gray-600 dark:text-gray-400 mt-1">Análisis consolidado de rentabilidad por vendedor</p>
                </div>
            </div>
        </div>
        ...
        <!-- Filtros -->
        @livewire('tenant.reports.filter-reports-generic', ['fields' => $filterFields], key('profitability-report-filters'))
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-900">
                        <tr>
                            ...
                            <th wire:click="sortBy('TotalCostoPromedio')"
                                ...
                                <div class="flex items-center justify-end gap-1">
                                    Total Costo Prom.
                                    @if($sortField === 'TotalCostoPromedio')
                                        @if($sortDirection === 'asc')
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"></path>
                                            </svg>
                            ...
                            <th wire:click="sortBy('TotalPrecioUltimo')"
                                class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 select-none">
                                <div class="flex items-center justify-end gap-1">
                                    Venta
                                    @if($sortField === 'TotalPrecioUltimo')
                                        @if($sortDirection === 'asc')
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"></path>
                                            </svg>
                            ...
                            <th wire:click="sortBy('TotalRentabilidad')"
                                class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 select-none">
                                <div class="flex items-center justify-end gap-1">
                                    Rentabilidad
                                    @if($sortField === 'TotalRentabilidad')
                                        @if($sortDirection === 'asc')
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"></path>
                                            </svg>
                            ...
                            <th wire:click="sortBy('PorcentajeRentabilidad')"
                                class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 select-none">
                                <div class="flex items-center justify-end gap-1">
                                    % Rentabilidad
                                    @if($sortField === 'PorcentajeRentabilidad')
                                        @if($sortDirection === 'asc')
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"></path>
                                            </svg>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        @else
                            @forelse ($profitabilityData as $record)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-bold">
                                        <span class="{{ $record->TotalRentabilidad > 0 ? 'text-green-600 dark:text-green-400' : ($record->TotalRentabilidad < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-gray-400') }}">
                                            ${{ number_format($record->TotalRentabilidad, 2) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-right font-bold">
                                        <span class="{{ $record->PorcentajeRentabilidad > 0 ? 'text-green-600 dark:text-green-400' : ($record->PorcentajeRentabilidad < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-gray-400') }}">
